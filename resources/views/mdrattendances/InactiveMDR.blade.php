...
<div class="content-header">
    <div class="text-center">
        <ul class="breadcrumbs">
            <li><i class="fa fa-home" aria-hidden="true"></i><a href="#"><h4 class="section-subtitle"><b>Inactive MDR Monthly Attendance</b></h4></a></li>
        </ul>
    </div>
</div>
    <div class="panel">
            <div class="panel-content">
                {{ Form::model(request()->old(),array('route' => array('mdrattendances.InactiveMDR'),'enctype'=>'multipart/form-data','class'=>'form-horizontal')) }}
                ...
                <div class="table-responsive">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><input align="center" style="width:50px; background-color: #99ff99;" type="text" name="eid_duty[]" id="eid_duty{{$i}}" class="rate" onblur="getWorkingDay({{$i}},'{{$data->inactiveDate}}')"></td>
                        <td><input align="center" style="width:50px;" type="text" name="working_days[]" id="working_days{{$i}}" class="rate" onblur="getAmount({{$i}},'{{$data->inactiveDate}}')" readonly="true"></td>
...
<script>
    ...
    function getInactiveDate(inactiveDate){
        let employerInactiveDate = inactiveDate;
        let employerJoinDate = new Date(employerInactiveDate);
        const parts = employerInactiveDate.split(/[- :]/);
        var month = parts[1];
        var year = parts[0];
...
function getAmount(id, inactiveDate){

    let inactiveDate_id = 'inactiveDate'+id
    ...
    let current_month_days = getInactiveDate(inactiveDate);
    //alert(current_month_days);
    let CurPayableDays = parseFloat((month_days - current_month_days)-unauthorized_leave);

    let d = new Date();
    let specificNumberOfFriday = specificDays('Friday',MONTHNAMES[d.getMonth()],d.getFullYear(),inactiveDate);
    
    if(current_month_days >0){

        let startDate = inactiveDate;

        if(current_month_days >15){
            //alert(current_month_days);
            $('#travelling_allowance'+id).val(parseFloat(working_days*120));
            $('#dearness_allowance'+id).val(parseFloat((working_days-meeting_days)*120));
            $('#mobile_bill'+id).val((parseFloat(500/month_days)*current_month_days).toFixed(2));
            $('#payable_days'+id).val(parseFloat(current_month_days));
            $('#salary'+id).val(parseFloat(((current_month_days)-unauthorized_leave)*(11000/month_days)).toFixed(2));
            ...
            $('#eid_duty_bill'+id).val(parseFloat(((11000/30)*2)*eid_duty).toFixed(2));
            $('#gross_salary'+id).val(parseFloat((((11000/month_days)*current_month_days) + ((500/month_days)*current_month_days) +(120)*working_days)+(120)*(working_days-meeting_days)+((11000/30)*(weekly_holiday + govt_holiday)) -((11000/month_days)*unauthorized_leave) + ((11000/30)*2)*eid_duty).toFixed(2));
        ...
        $('#govt_holiday_bill'+id).val(parseFloat((11000/30)*govt_holiday).toFixed(2));
        $('#eid_duty_bill'+id).val(parseFloat(((11000/30)*2)*eid_duty).toFixed(2));
        $('#gross_salary'+id).val(parseFloat(((11000)+(120)*current_month_days)+(120)*(working_days-meeting_days)+((11000/30)*(weekly_holiday + govt_holiday + ((500/month_days)*current_month_days))) -((11000/month_days)*unauthorized_leave) + ((11000/30)*2)*eid_duty).toFixed(2));
    } 
       
}

        function specificDays(dayName, monthName, year, inactiveDate='') {
          // set names
          let inactiveDateCal = new Date(inactiveDate); 
 ...
 function getWorkingDay(id, inactiveDate){
    ...
    let CurPayableDays = getInactiveDate(inactiveDate);
    //alert(CurPayableDays);
    
    let d = new Date();
    let specificNumberOfFriday_new = specificDays('Friday',MONTHNAMES[d.getMonth()],d.getFullYear(),inactiveDate);
    //alert(specificNumberOfFriday_new);

    let specificNumberOfFriday_month = specificDays('Friday',MONTHNAMES[d.getMonth()],d.getFullYear());
    //alert(specificNumberOfFriday_month);
